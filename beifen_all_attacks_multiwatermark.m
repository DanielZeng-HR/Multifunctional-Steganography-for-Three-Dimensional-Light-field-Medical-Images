...
IMG_Gaussian = IMG;
sigma = 2;  % 设置高斯滤波的标准差
Embed_filtered = imgaussfilt(real(IMG_Gaussian), sigma,'FilterSize', 3);  % , 'FilterSize', 3: 3*3滤波
figure;
subplot(2,2,1); imshow(Embed_filtered); title('高斯滤波图像');

len=1;
for i=1:m
    for j=1:n
        if(len<=txt_len)
            if(Embed_filtered(i,j)==P)
                txtbs_Gaussian(len)=0;
                len=len+1;
            elseif(Embed_filtered(i,j)==P+move)
                txtbs_Gaussian(len)=1;
%                 Img(i,j)=P;
                len=len+1;
            end
        end
...
txtb_Gaussian=reshape(txtbs_Gaussian,8,txt_len/8);%一维变二维
txtb_Gaussian=txtb_Gaussian';
txt_out_Gaussian=num2str(txtb_Gaussian);%转为二进制字符串
txt_out_Gaussian=bin2dec(txt_out_Gaussian);%转为0-255
txt_out_Gaussian = char(txt_out_Gaussian)';

%% cropping
IMG_Cropped = IMG;
IMG_Cropped(1:80, 1:80) = 255;
subplot(2,2,2); imshow(IMG_Cropped); title('裁剪图像');

len=1;
for i=1:m
    for j=1:n
        if(len<=txt_len)
            if(IMG_Cropped(i,j)==P)
                txtbs_Cropped(len)=0;
                len=len+1;
            elseif(IMG_Cropped(i,j)==P+move)
                txtbs_Cropped(len)=1;
%                 Img(i,j)=P;
                len=len+1;
            end
...
txtb_Cropped=reshape(txtbs_Cropped,8,txt_len/8);%一维变二维
txtb_Cropped=txtb_Cropped';
txt_out_Cropped=num2str(txtb_Cropped);%转为二进制字符串
txt_out_Cropped=bin2dec(txt_out_Cropped);%转为0-255
txt_out_Cropped = char(txt_out_Cropped)';
...
txtb_JPEG=reshape(txtbs_JPEG,8,txt_len/8);%一维变二维
txtb_JPEG=txtb_JPEG';
txt_out_JPEG=num2str(txtb_JPEG);%转为二进制字符串
txt_out_JPEG=bin2dec(txt_out_JPEG);%转为0-255
txt_out_JPEG = char(txt_out_JPEG)';
...
subplot(2,2,4); imshow(IMG_Rescaling); title('缩放图像');

len=1;
for i=1:m
    for j=1:n
        if(len<=txt_len)
            if(IMG_Rescaling(i,j)==P)
                txtbs_Rescaling(len)=0;
                len=len+1;
            elseif(IMG_Rescaling(i,j)==P+move)
                txtbs_Rescaling(len)=1;
%                 Img(i,j)=P;
                len=len+1;
            end
        end
...
txtb_Rescaling=reshape(txtbs_Rescaling,8,txt_len/8);%一维变二维
txtb_Rescaling=txtb_Rescaling';
txt_out_Rescaling=num2str(txtb_Rescaling);%转为二进制字符串
txt_out_Rescaling=bin2dec(txt_out_Rescaling);%转为0-255
txt_out_Rescaling = char(txt_out_Rescaling)';

%% rotate
IMG_Rotated = imrotate(Img, 90);
figure;
subplot(2,2,1); imshow(IMG_Rotated); title('旋转图像');

len=1;
for i=1:m
    for j=1:n
        if(len<=txt_len)
            if(IMG_Rotated(i,j)==P)
                txtbs_Rotated(len)=0;
                len=len+1;
            elseif(IMG_Rotated(i,j)==P+move)
                txtbs_Rotated(len)=1;
%                 Img(i,j)=P;
                len=len+1;
            end
...
txtb_Rotated=reshape(txtbs_Rotated,8,txt_len/8);%一维变二维
txtb_Rotated=txtb_Rotated';
txt_out_Rotated=num2str(txtb_Rotated);%转为二进制字符串
txt_out_Rotated=bin2dec(txt_out_Rotated);%转为0-255
txt_out_Rotated = char(txt_out_Rotated)';
