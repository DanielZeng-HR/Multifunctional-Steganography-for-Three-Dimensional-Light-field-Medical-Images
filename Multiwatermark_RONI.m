...
root_num = floor(sqrt(seq_length))-1;  % 计算最接近的完全平方数
...
imwrite(reshaped_IMG_RONI,'D:\X.W.Li materials\InIm鲁邦水印\隐写代码完整版\robust_host.png');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% no attack
[Extracted_logo] = extract_watermark_RONI(IMG, rows_RONI, cols_RONI,...
    RONI_length, root_num, U1, V1,S,af, setLength_logo, Embedded_RONI);

subplot(2,2,4);
imshow(uint8(Extracted_logo));
imwrite(uint8(Extracted_logo),'D:\X.W.Li materials\InIm鲁邦水印\隐写代码完整版\CII program1\dual-SVD-十所\no attack\Extracted_logo.tif');
title('Extracted logo');

fprintf('SSIM for no attack: %.4f\n',  ssim(watermark_logo,uint8(Extracted_logo)));
fprintf('PSNR for no attack: %.4f\n',  psnr(watermark_logo, uint8(Extracted_logo)));
fprintf('NCC for no attack: %.4f\n',  ncc(watermark_logo, uint8(Extracted_logo)));
...
[Extracted_logo] = extract_watermark_RONI(Gaussian_filtered, rows_RONI, cols_RONI,...
    RONI_length, root_num, U1, V1,S,af, setLength_logo, Embedded_RONI);

subplot(2,2,2);
imshow(uint8(Extracted_logo));
imwrite(uint8(Extracted_logo),'D:\X.W.Li materials\InIm鲁邦水印\隐写代码完整版\CII program1\dual-SVD-十所\Gaussian_filter\Extracted_logo.tif');
title('高斯滤波提取logo');
fprintf('SSIM for Guassain filtering: %.4f\n',  ssim(watermark_logo, uint8(Extracted_logo)));
fprintf('PSNR for Guassain filtering: %.4f\n',  psnr(watermark_logo, uint8(Extracted_logo)));
fprintf('NCC for Guassain filtering: %.4f\n',  ncc(watermark_logo, uint8(Extracted_logo)));
%% Guassain noise 
...
[Extracted_logo] = extract_watermark_RONI(Gaussian_noised, rows_RONI, cols_RONI,...
    RONI_length, root_num, U1, V1,S,af, setLength_logo, Embedded_RONI);

subplot(2,2,4);
imshow(uint8(Extracted_logo));
imwrite(uint8(Extracted_logo),'D:\X.W.Li materials\InIm鲁邦水印\隐写代码完整版\CII program1\dual-SVD-十所\Gaussian_noise\Extracted_logo.tif');
title('高斯噪声提取logo');
fprintf('SSIM for Guassain noise : %.4f\n',  ssim(watermark_logo, uint8(Extracted_logo)));
fprintf('PSNR for Guassain noise: %.4f\n',  psnr(watermark_logo, uint8(Extracted_logo)));
fprintf('NCC for Guassain noise: %.4f\n',  ncc(watermark_logo, uint8(Extracted_logo)));
%% cropping ROI
Cropped_value = 255;
ROI_cropped =  IMG;
...
[Extracted_logo] = extract_watermark_RONI(ROI_cropped, rows_RONI, cols_RONI,...
    RONI_length, root_num, U1, V1,S,af, setLength_logo, Embedded_RONI);

subplot(2,2,2);
imshow(uint8(Extracted_logo));
imwrite(uint8(Extracted_logo),'D:\X.W.Li materials\InIm鲁邦水印\隐写代码完整版\CII program1\dual-SVD-十所\cropping ROI\Extracted_logo.tif');
title('ROI裁剪logo');
fprintf('SSIM for cropping ROI : %.4f\n',  ssim(watermark_logo, uint8(Extracted_logo)));
fprintf('PSNR for cropping ROI: %.4f\n',  psnr(watermark_logo, uint8(Extracted_logo)));
fprintf('NCC for cropping ROI: %.4f\n',  ncc(watermark_logo, uint8(Extracted_logo)));
%% JPEG 
Embed_uint8 = IMG;
...
[Extracted_logo] = extract_watermark_RONI(IMG_JPEG, rows_RONI, cols_RONI,...
    RONI_length, root_num, U1, V1,S,af, setLength_logo, Embedded_RONI);

subplot(2,2,4);
imshow(uint8(Extracted_logo));
imwrite(uint8(Extracted_logo),'D:\X.W.Li materials\InIm鲁邦水印\隐写代码完整版\CII program1\dual-SVD-十所\JPEG compression\Extracted_logo.tif');
title('JPEG压缩logo');
fprintf('SSIM for JPEG: %.4f\n',  ssim(watermark_logo, uint8(Extracted_logo)));
fprintf('PSNR for JPEG: %.4f\n',  psnr(watermark_logo, uint8(Extracted_logo)));
fprintf('NCC for JPEG: %.4f\n',  ncc(watermark_logo, uint8(Extracted_logo)));
%% rescaling 
...
[Extracted_logo] = extract_watermark_RONI(IMG_Rescaling, rows_RONI, cols_RONI,...
    RONI_length, root_num, U1, V1,S,af, setLength_logo, Embedded_RONI);

subplot(2,2,2);
imshow(uint8(Extracted_logo));
imwrite(uint8(Extracted_logo),'D:\X.W.Li materials\InIm鲁邦水印\隐写代码完整版\CII program1\dual-SVD-十所\Rescaling\Extracted_logo.tif');
title('rescaling logo');
fprintf('SSIM for rescaling: %.4f\n',  ssim(watermark_logo, uint8(Extracted_logo)));
fprintf('PSNR for rescaling: %.4f\n',  psnr(watermark_logo, uint8(Extracted_logo)));
fprintf('NCC for rescaling: %.4f\n',  ncc(watermark_logo, uint8(Extracted_logo)));
%% rotate
IMG_Rotated = imrotate(IMG, 40);
subplot(2,2,3); imshow(IMG_Rotated); title('旋转图像');

[Extracted_logo] = extract_watermark_RONI(IMG_Rotated, rows_RONI, cols_RONI,...
    RONI_length, root_num, U1, V1,S,af, setLength_logo, Embedded_RONI);

subplot(2,2,4);
imshow(uint8(Extracted_logo));
imwrite(uint8(Extracted_logo),'D:\X.W.Li materials\InIm鲁邦水印\隐写代码完整版\CII program1\dual-SVD-十所\rotate\Extracted_logo.tif');
title('rorated logo');
fprintf('SSIM for rotate: %.4f\n',  ssim(watermark_logo, uint8(Extracted_logo)));
fprintf('PSNR for rotate: %.4f\n',  psnr(watermark_logo, uint8(Extracted_logo)));
fprintf('NCC for rotate: %.4f\n',  ncc(watermark_logo, uint8(Extracted_logo)));
...
[Extracted_logo] = extract_watermark_RONI(Speckle_noised, rows_RONI, cols_RONI,...
    RONI_length, root_num, U1, V1,S,af, setLength_logo, Embedded_RONI);

subplot(2,2,2);
imshow(uint8(Extracted_logo));
imwrite(uint8(Extracted_logo),'D:\X.W.Li materials\InIm鲁邦水印\隐写代码完整版\CII program1\dual-SVD-十所\speckle noise\Extracted_logo.tif');
title('speckle noised logo');
fprintf('SSIM for speckle noise: %.4f\n',  ssim(watermark_logo, uint8(Extracted_logo)));
fprintf('PSNR for speckle noise: %.4f\n',  psnr(watermark_logo, uint8(Extracted_logo)));
fprintf('NCC for speckle noise: %.4f\n',  ncc(watermark_logo, uint8(Extracted_logo)));
%% sharpening
...
[Extracted_logo] = extract_watermark_RONI(sharpening, rows_RONI, cols_RONI,...
    RONI_length, root_num, U1, V1,S,af, setLength_logo, Embedded_RONI);

subplot(2,2,4);
imshow(uint8(Extracted_logo));
imwrite(uint8(Extracted_logo),'D:\X.W.Li materials\InIm鲁邦水印\隐写代码完整版\CII program1\dual-SVD-十所\sharpening\Extracted_logo.tif');
title('Sharpening logo');
fprintf('SSIM for sharpening: %.4f\n',  ssim(watermark_logo, uint8(Extracted_logo)));
fprintf('PSNR for sharpening: %.4f\n',  psnr(watermark_logo, uint8(Extracted_logo)));
fprintf('NCC for sharpening: %.4f\n',  ncc(watermark_logo, uint8(Extracted_logo)));
%% salt pepper noise
...
[Extracted_logo] = extract_watermark_RONI(salt_noised, rows_RONI, cols_RONI,...
    RONI_length, root_num, U1, V1,S,af, setLength_logo, Embedded_RONI);

subplot(2,2,2);
imshow(uint8(Extracted_logo));
imwrite(uint8(Extracted_logo),'D:\X.W.Li materials\InIm鲁邦水印\隐写代码完整版\CII program1\dual-SVD-十所\salt pepper noise\Extracted_logo.tif');
title('salt & pepper logo');
fprintf('SSIM for salt pepper noise: %.4f\n',  ssim(watermark_logo, uint8(Extracted_logo)));
fprintf('PSNR for salt pepper noise: %.4f\n',  psnr(watermark_logo, uint8(Extracted_logo)));
fprintf('NCC for salt pepper noise: %.4f\n',  ncc(watermark_logo, uint8(Extracted_logo)));
...
[Extracted_logo] = extract_watermark_RONI(RONI_cropped, rows_RONI, cols_RONI,...
    RONI_length, root_num, U1, V1,S,af, setLength_logo, Embedded_RONI);

subplot(2,2,4);
imshow(uint8(Extracted_logo));
imwrite(uint8(Extracted_logo),'D:\X.W.Li materials\InIm鲁邦水印\隐写代码完整版\CII program1\dual-SVD-十所\cropping RONI\Extracted_logo.tif');
title('RONI裁剪logo');
fprintf('SSIM for cropping RONI: %.4f\n',  ssim(watermark_logo, uint8(Extracted_logo)));
fprintf('PSNR for cropping RONI: %.4f\n',  psnr(watermark_logo, uint8(Extracted_logo)));
fprintf('NCC for cropping RONI: %.4f\n',  ncc(watermark_logo, uint8(Extracted_logo)));
